...
function memulihkan_kata_sandi_file_zip_brute(){
	...
	john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${file_hash_file_zip}"
	if [[ -f "${pot_file_zip}" ]]; then
		if [[ $(cat "${pot_file_zip}" | grep -o ":") ]]; then
			kata_sandi_file_zip=$(cat "${pot_file_zip}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_rar_brute(){
	...
	echo ""
	format_file_rar="RAR5"
	john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${file_hash_file_rar}"
	if [[ -f "${pot_file_rar}" ]]; then
		if [[ $(cat "${pot_file_rar}" | grep -o ":") ]]; then
			kata_sandi_file_rar=$(cat "${pot_file_rar}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_7z_brute(){
	...
	echo ""
	format_file_7z="7z"
	john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${file_hash_file_7z}"
	if [[ -f "${pot_file_7z}" ]]; then
		if [[ $(cat "${pot_file_7z}" | grep -o ":") ]]; then
			kata_sandi_file_7z=$(cat "${pot_file_7z}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_pdf_brute(){
	...
	echo ""
	format_file_pdf="PDF"
	john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${file_hash_file_pdf}"
	if [[ -f "${pot_file_pdf}" ]]; then
		if [[ $(cat "${pot_file_pdf}" | grep -o ":") ]]; then
			kata_sandi_file_pdf=$(cat "${pot_file_pdf}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_office_brute(){
	...
	echo ""
	format_file_office="Office"
	john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${file_hash_file_office}"
	if [[ -f "${pot_file_office}" ]]; then
		if [[ $(cat "${pot_file_office}" | grep -o ":") ]]; then
			kata_sandi_file_office=$(cat "${pot_file_office}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_zip_mask(){
	...
	john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${file_hash_file_zip}"
	if [[ -f "${pot_file_zip}" ]]; then
		if [[ $(cat "${pot_file_zip}" | grep -o ":") ]]; then
			kata_sandi_file_zip=$(cat "${pot_file_zip}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_rar_mask(){
	...
	echo ""
	format_file_rar="RAR5"
	john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${file_hash_file_rar}"
	if [[ -f "${pot_file_rar}" ]]; then
		if [[ $(cat "${pot_file_rar}" | grep -o ":") ]]; then
			kata_sandi_file_rar=$(cat "${pot_file_rar}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_7z_mask(){
	...
	echo ""
	format_file_7z="7z"
	john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${file_hash_file_7z}"
	if [[ -f "${pot_file_7z}" ]]; then
		if [[ $(cat "${pot_file_7z}" | grep -o ":") ]]; then
			kata_sandi_file_7z=$(cat "${pot_file_7z}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_pdf_mask(){
	...
	echo ""
	format_file_pdf="PDF"
	john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${file_hash_file_pdf}"
	if [[ -f "${pot_file_pdf}" ]]; then
		if [[ $(cat "${pot_file_pdf}" | grep -o ":") ]]; then
			kata_sandi_file_pdf=$(cat "${pot_file_pdf}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_office_mask(){
	...
	echo ""
	format_file_office="Office"
	john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${file_hash_file_office}"
	if [[ -f "${pot_file_office}" ]]; then
		if [[ $(cat "${pot_file_office}" | grep -o ":") ]]; then
			kata_sandi_file_office=$(cat "${pot_file_office}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_zip_prince(){
	...
	john --prince="${file_wordlist}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${file_hash_file_zip}"
	if [[ -f "${pot_file_zip}" ]]; then
		if [[ $(cat "${pot_file_zip}" | grep -o ":") ]]; then
			kata_sandi_file_zip=$(cat "${pot_file_zip}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_rar_prince(){
	...
	echo ""
	format_file_rar="RAR5"
	john --prince="${file_wordlist}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${file_hash_file_rar}"
	if [[ -f "${pot_file_rar}" ]]; then
		if [[ $(cat "${pot_file_rar}" | grep -o ":") ]]; then
			kata_sandi_file_rar=$(cat "${pot_file_rar}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_7z_prince(){
	...
	echo ""
	format_file_7z="7z"
	john --prince="${file_wordlist}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${file_hash_file_7z}"
	if [[ -f "${pot_file_7z}" ]]; then
		if [[ $(cat "${pot_file_7z}" | grep -o ":") ]]; then
			kata_sandi_file_7z=$(cat "${pot_file_7z}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_pdf_prince(){
	...
	echo ""
	format_file_pdf="PDF"
	john --prince="${file_wordlist}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${file_hash_file_pdf}"
	if [[ -f "${pot_file_pdf}" ]]; then
		if [[ $(cat "${pot_file_pdf}" | grep -o ":") ]]; then
			kata_sandi_file_pdf=$(cat "${pot_file_pdf}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_office_prince(){
	...
	echo ""
	format_file_office="Office"
	john --prince="${file_wordlist}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${file_hash_file_office}"
	if [[ -f "${pot_file_office}" ]]; then
		if [[ $(cat "${pot_file_office}" | grep -o ":") ]]; then
			kata_sandi_file_office=$(cat "${pot_file_office}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_zip_subsets(){
	...
	john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${file_hash_file_zip}"
	if [[ -f "${pot_file_zip}" ]]; then
		if [[ $(cat "${pot_file_zip}" | grep -o ":") ]]; then
			kata_sandi_file_zip=$(cat "${pot_file_zip}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_rar_subsets(){
	...
	echo ""
	format_file_rar="RAR5"
	john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${file_hash_file_rar}"
	if [[ -f "${pot_file_rar}" ]]; then
		if [[ $(cat "${pot_file_rar}" | grep -o ":") ]]; then
			kata_sandi_file_rar=$(cat "${pot_file_rar}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_7z_subsets(){
	...
	echo ""
	format_file_7z="7z"
	john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${file_hash_file_7z}"
	if [[ -f "${pot_file_7z}" ]]; then
		if [[ $(cat "${pot_file_7z}" | grep -o ":") ]]; then
			kata_sandi_file_7z=$(cat "${pot_file_7z}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_pdf_subsets(){
	...
	echo ""
	format_file_pdf="PDF"
	john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${file_hash_file_pdf}"
	if [[ -f "${pot_file_pdf}" ]]; then
		if [[ $(cat "${pot_file_pdf}" | grep -o ":") ]]; then
			kata_sandi_file_pdf=$(cat "${pot_file_pdf}" | cut -d ":" -f 2)
                        echo ""
...
function memulihkan_kata_sandi_file_office_subsets(){
	...
	echo ""
	format_file_office="Office"
	john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${file_hash_file_office}"
	if [[ -f "${pot_file_office}" ]]; then
		if [[ $(cat "${pot_file_office}" | grep -o ":") ]]; then
			kata_sandi_file_office=$(cat "${pot_file_office}" | cut -d ":" -f 2)
                        echo ""
