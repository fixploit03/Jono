...
function ekstrak_hash_file_zip(){
	echo -e "${p}[${b}*${p}] Mengekstrak hash file ZIP...${r}"
        tunggu
        hash_file_zip=$(zip2john "${file_zip}" 2>&1)
...
function ekstrak_hash_file_rar(){
	echo -e "${p}[${b}*${p}] Mengekstrak hash file RAR...${r}"
        tunggu
        hash_file_rar=$(rar2john "${file_rar}" 2>&1)
...
function ekstrak_hash_file_7z(){
        ...
        tunggu
        hash_file_7z=$(7z2john "${file_7z}" 2>&1)
...
function ekstrak_hash_file_pdf(){
	echo -e "${p}[${b}*${p}] Mengekstrak hash file PDF...${r}"
        tunggu
        hash_file_pdf=$(pdf2john "${file_pdf}" 2>&1)
...
function ekstrak_hash_file_office(){
	echo -e "${p}[${b}*${p}] Mengekstrak hash file Office...${r}"
        tunggu
        hash_file_office=$(office2john "${file_office}" 2>&1)
...
function memulihkan_kata_sandi_file_zip(){
	...
	while true; do
		...
		if [[ "${pilih_teknik_zip}" == "0" ]]; then
			...
			echo ""
			john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_zip}"
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file ZIP...\e[0m'
			echo ""
			john --wordlist="${file_wordlist}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_zip}"
			if [[ -f "${pot_file_zip}" ]]; then
				if [[ $(cat "${pot_file_zip}" | grep -o ":") ]]; then
					kata_sandi_file_zip=$(cat "${pot_file_zip}" | cut -d ":" -f 2)
                      		  	echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file ZIP...\e[0m'
			echo ""
			john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_zip}"
			if [[ -f "${pot_file_zip}" ]]; then
				if [[ $(cat "${pot_file_zip}" | grep -o ":") ]]; then
					kata_sandi_file_zip=$(cat "${pot_file_zip}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file ZIP...\e[0m'
			echo ""
			john --prince="${file_wordlist}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_zip}"
			if [[ -f "${pot_file_zip}" ]]; then
				if [[ $(cat "${pot_file_zip}" | grep -o ":") ]]; then
					kata_sandi_file_zip=$(cat "${pot_file_zip}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file ZIP...\e[0m'
			echo ""
			john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_zip}" --pot="${pot_file_zip}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_zip}"
			if [[ -f "${pot_file_zip}" ]]; then
				if [[ $(cat "${pot_file_zip}" | grep -o ":") ]]; then
					kata_sandi_file_zip=$(cat "${pot_file_zip}" | cut -d ":" -f 2)
		                        echo ""
...
function memulihkan_kata_sandi_file_rar(){
	...
	while true; do
		...
		if [[ "${pilih_teknik_rar}" == "0" ]]; then
			...
			echo ""
			john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_rar}"
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file RAR...\e[0m'
			echo ""
			john --wordlist="${file_wordlist}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_rar}"
			if [[ -f "${pot_file_rar}" ]]; then
				if [[ $(cat "${pot_file_rar}" | grep -o ":") ]]; then
					kata_sandi_file_rar=$(cat "${pot_file_rar}" | cut -d ":" -f 2)
                      		  	echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file RAR...\e[0m'
			echo ""
			john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_rar}"
			if [[ -f "${pot_file_rar}" ]]; then
				if [[ $(cat "${pot_file_rar}" | grep -o ":") ]]; then
					kata_sandi_file_rar=$(cat "${pot_file_rar}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file RAR...\e[0m'
			echo ""
			john --prince="${file_wordlist}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_rar}"
			if [[ -f "${pot_file_rar}" ]]; then
				if [[ $(cat "${pot_file_rar}" | grep -o ":") ]]; then
					kata_sandi_file_rar=$(cat "${pot_file_rar}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file RAR...\e[0m'
			echo ""
			john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_rar}" --pot="${pot_file_rar}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_rar}"
			if [[ -f "${pot_file_rar}" ]]; then
				if [[ $(cat "${pot_file_rar}" | grep -o ":") ]]; then
					kata_sandi_file_rar=$(cat "${pot_file_rar}" | cut -d ":" -f 2)
		                        echo ""
...
function memulihkan_kata_sandi_file_7z(){
	...
	while true; do
		...
		if [[ "${pilih_teknik_7z}" == "0" ]]; then
			...
			echo ""
			john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_7z}"
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file 7z...\e[0m'
			echo ""
			john --wordlist="${file_wordlist}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_7z}"
			if [[ -f "${pot_file_7z}" ]]; then
				if [[ $(cat "${pot_file_7z}" | grep -o ":") ]]; then
					kata_sandi_file_7z=$(cat "${pot_file_7z}" | cut -d ":" -f 2)
                      		  	echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file 7z...\e[0m'
			echo ""
			john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_7z}"
			if [[ -f "${pot_file_7z}" ]]; then
				if [[ $(cat "${pot_file_7z}" | grep -o ":") ]]; then
					kata_sandi_file_7z=$(cat "${pot_file_7z}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file 7z...\e[0m'
			echo ""
			john --prince="${file_wordlist}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_7z}"
			if [[ -f "${pot_file_7z}" ]]; then
				if [[ $(cat "${pot_file_7z}" | grep -o ":") ]]; then
					kata_sandi_file_7z=$(cat "${pot_file_7z}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file 7z...\e[0m'
			echo ""
			john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_7z}" --pot="${pot_file_7z}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_7z}"
			if [[ -f "${pot_file_7z}" ]]; then
				if [[ $(cat "${pot_file_7z}" | grep -o ":") ]]; then
					kata_sandi_file_7z=$(cat "${pot_file_7z}" | cut -d ":" -f 2)
		                        echo ""
...
function memulihkan_kata_sandi_file_pdf(){
	...
	while true; do
		...
		if [[ "${pilih_teknik_pdf}" == "0" ]]; then
			...
			echo ""
			john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_pdf}"
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file PDF...\e[0m'
			echo ""
			john --wordlist="${file_wordlist}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_pdf}"
			if [[ -f "${pot_file_pdf}" ]]; then
				if [[ $(cat "${pot_file_pdf}" | grep -o ":") ]]; then
					kata_sandi_file_pdf=$(cat "${pot_file_pdf}" | cut -d ":" -f 2)
                      		  	echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file PDF...\e[0m'
			echo ""
			john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_pdf}"
			if [[ -f "${pot_file_pdf}" ]]; then
				if [[ $(cat "${pot_file_pdf}" | grep -o ":") ]]; then
					kata_sandi_file_pdf=$(cat "${pot_file_pdf}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file PDF...\e[0m'
			echo ""
			john --prince="${file_wordlist}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_pdf}"
			if [[ -f "${pot_file_pdf}" ]]; then
				if [[ $(cat "${pot_file_pdf}" | grep -o ":") ]]; then
					kata_sandi_file_pdf=$(cat "${pot_file_pdf}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file PDF...\e[0m'
			echo ""
			john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_pdf}" --pot="${pot_file_pdf}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_pdf}"
			if [[ -f "${pot_file_pdf}" ]]; then
				if [[ $(cat "${pot_file_pdf}" | grep -o ":") ]]; then
					kata_sandi_file_pdf=$(cat "${pot_file_pdf}" | cut -d ":" -f 2)
		                        echo ""
...
function memulihkan_kata_sandi_file_office(){
	...
	while true; do
		...
		if [[ "${pilih_teknik_office}" == "0" ]]; then
			...
			echo ""
			john --incremental --min-length="${min}" --max-length="${maks}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_office}"
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file Office...\e[0m'
			echo ""
			john --wordlist="${file_wordlist}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_office}"
			if [[ -f "${pot_file_office}" ]]; then
				if [[ $(cat "${pot_file_office}" | grep -o ":") ]]; then
					kata_sandi_file_office=$(cat "${pot_file_office}" | cut -d ":" -f 2)
                      		  	echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file Office...\e[0m'
			echo ""
			john --mask="${pola_mask}" --min-length="${min}" --max-length="${maks}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_office}"
			if [[ -f "${pot_file_office}" ]]; then
				if [[ $(cat "${pot_file_office}" | grep -o ":") ]]; then
					kata_sandi_file_office=$(cat "${pot_file_office}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file Office...\e[0m'
			echo ""
			john --prince="${file_wordlist}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_office}"
			if [[ -f "${pot_file_office}" ]]; then
				if [[ $(cat "${pot_file_office}" | grep -o ":") ]]; then
					kata_sandi_file_office=$(cat "${pot_file_office}" | cut -d ":" -f 2)
		                        echo ""
			...
			read -p $'\e[1;37mTekan [\e[1;32mEnter\e[1;37m] untuk memulai proses pemulihan kata sandi file Office...\e[0m'
			echo ""
			john --subsets="${kombinasi}" --min-length="${min}" --max-length="${maks}" --format="${format_file_office}" --pot="${pot_file_office}" --verbosity=6 --progress-every=1 "${nama_file_hash_file_office}"
			if [[ -f "${pot_file_office}" ]]; then
				if [[ $(cat "${pot_file_office}" | grep -o ":") ]]; then
					kata_sandi_file_office=$(cat "${pot_file_office}" | cut -d ":" -f 2)
		                        echo ""
